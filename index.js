...
app.put("/equipos/:id", (req, res) => {
  ...
  const qFechaMantenimientoActual = "SELECT fecha_mantenimiento FROM mantenimiento_equipo WHERE id_equipo = ?";
  db.query(qFechaMantenimientoActual, [equipoId], (errFechaMantenimiento, resultFechaMantenimiento) => {
    if (errFechaMantenimiento) return res.json(errFechaMantenimiento);

    const fechaMantenimientoActual = resultFechaMantenimiento[0] ? resultFechaMantenimiento[0].fecha_mantenimiento : null;
    ...
    db.query(qActualizarEquipo, valuesActualizarEquipo, (errActualizarEquipo, dataActualizarEquipo) => {
...
app.get("/equipos/:id/mantenimiento", (req, res) => {
  const equipoId = req.params.id;
  const q = "SELECT * FROM mantenimiento_equipo WHERE id_equipo = ? ORDER BY fecha_mantenimiento DESC LIMIT 1";
